...
import { Eye, ShoppingBag, TrendingDown, Target, Activity } from "lucide-react";
import { useTranslation } from "react-i18next";

const stats = [
  { key: "visibility", Icon: Eye },
  { key: "directOrders", Icon: ShoppingBag },
  { key: "lowerSpend", Icon: TrendingDown },
  { key: "roi", Icon: Target },
  { key: "tracking", Icon: Activity },
];

export default function SmarterAdsSection() {
              ...
              >
                <div>
                  ...
                  <h4 className="font-bold text-gray-900 mt-2 lg:mt-3 text-xs sm:text-sm">
                    {t(`googleAds.smarterAds.stats.${key}.title`)}
                  </h4>
                </div>
                <p className="text-[10px] sm:text-xs text-gray-500 mt-1">
                  {t(`googleAds.smarterAds.stats.${key}.desc`)}
                </p>
              </motion.div>
            ))}
          </div>
