...
import { BadgeCheck, Lightbulb, MonitorCheck, Clock, TrendingUp } from "lucide-react";
import { useTranslation } from "react-i18next";

const stats = [
  { key: "tracking", Icon: BadgeCheck },
  { key: "insights", Icon: Lightbulb },
  { key: "monitoring", Icon: MonitorCheck },
  { key: "saveTime", Icon: Clock },
  { key: "growth", Icon: TrendingUp },
];

export default function BetterInsightsSection() {
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4 lg:gap-6 mt-6 lg:mt-8">
            {stats.map(({ key, Icon }) => (
              <motion.div
                key={key}
                whileHover={{ y: -4 }}
                className="bg-white border border-gray-100 rounded-2xl lg:rounded-3xl p-4 lg:p-6 text-center shadow-[0_15px_50px_rgba(0,0,0,0.06)] h-full flex flex-col justify-between"
              >
                <div>
                  <Icon className="text-red-600 mx-auto" size={20} />
                  <h4 className="font-bold text-gray-900 mt-2 lg:mt-3 text-xs sm:text-sm">
                    {t(`googleAnalytics.betterInsights.stats.${key}.title`)}
                  </h4>
                </div>
                <p className="text-[10px] sm:text-xs text-gray-500 mt-1">
                  {t(`googleAnalytics.betterInsights.stats.${key}.desc`)}
                </p>
              </motion.div>
            ))}
          </div>
